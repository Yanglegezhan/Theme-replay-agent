...
class KaipanlaDataSource:
    ...
    def _enrich_stock_data(self, stock_code: str, stock_data: dict, api_date: str = None) -> dict:
        """从多个数据源获取缺失字段并补全股票数据
        ...
        """
        try:
            # 方法1: 使用 AkShare 涨停池接口获取完整数据（推荐）
            try:
                import akshare as ak
                from datetime import datetime

                # 获取涨停池数据
                date_str = api_date.replace('-', '') if api_date else datetime.now().strftime('%Y%m%d')
                zt_pool = ak.stock_zt_pool_em(date=date_str)

                if zt_pool is not None and not zt_pool.empty:
                    # 查找匹配的股票
                    stock_code_normalized = self._normalize_stock_code(stock_code)
                    matched = zt_pool[zt_pool['代码'] == stock_code_normalized]

                    if not matched.empty:
                        row = matched.iloc[0]

                        # 从涨停池获取字段（AkShare字段名映射）
                        # 首次封板时间
                        if '首次封板时间' in row and pd.notna(row['首次封板时间']):
                            seal_time = str(row['首次封板时间'])
                            # 格式化时间（如 092500 -> 09:25:00）
                            if seal_time.isdigit() and len(seal_time) == 6:
                                seal_time = f"{seal_time[:2]}:{seal_time[2:4]}:{seal_time[4:]}"
                            stock_data['首次封板时间'] = seal_time

                        # 最后封板时间
                        if '最后封板时间' in row and pd.notna(row['最后封板时间']):
                            last_seal_time = str(row['最后封板时间'])
                            if last_seal_time.isdigit() and len(last_seal_time) == 6:
                                last_seal_time = f"{last_seal_time[:2]}:{last_seal_time[2:4]}:{last_seal_time[4:]}"
                            stock_data['最终封板时间'] = last_seal_time

                        # 炸板次数（注意：炸板次数不等于开板次数，但可以作为参考）
                        if '炸板次数' in row and pd.notna(row['炸板次数']):
                            stock_data['炸板次数'] = int(row['炸板次数'])
                            # 开板次数暂用炸板次数代替（实际可能需要其他数据源）
                            stock_data['开板次数'] = int(row['炸板次数'])

                        # 换手率
                        if '换手率' in row and pd.notna(row['换手率']):
                            stock_data['换手率'] = float(row['换手率'])

                        # 成交额
                        if '成交额' in row and pd.notna(row['成交额']):
                            turnover_val = float(row['成交额'])
                            stock_data['成交额'] = turnover_val
                            stock_data['_turnover'] = turnover_val

                        # 封板资金（封单额）
                        if '封板资金' in row and pd.notna(row['封板资金']):
                            seal_val = float(row['封板资金'])
                            stock_data['封单额'] = seal_val
                            stock_data['封单额_数值'] = seal_val
                        ...
                        if '连板数' in row and pd.notna(row['连板数']):
                            ...
                            if board_num == 1:
                                ...
                                stock_data['连板天数'] = f'{board_num}连板'

                        # 涨停统计（如 "3/3" 表示3连板/3板）
                        if '涨停统计' in row and pd.notna(row['涨停统计']):
                            stock_data['涨停统计'] = str(row['涨停统计'])

                        logger.debug(f"从AkShare涨停池获取{stock_code}数据成功")

            except Exception as e:
                logger.debug(f"从AkShare涨停池获取数据失败: {e}")

            # 方法2: 使用 get_stock_intraday 获取成交额和涨跌幅（补充）
            if not stock_data.get('成交额'):
                try:
                    intraday_data = self._retry_request(
                        self.crawler.get_stock_intraday,
                    ...
                    if intraday_data:
                        if 'total_turnover' in intraday_data and intraday_data['total_turnover']:
                            stock_data['成交额'] = float(intraday_data['total_turnover'])
                            stock_data['_turnover'] = float(intraday_data['total_turnover'])

                        if 'px_change_rate' in intraday_data:
                            stock_data['涨跌幅'] = float(intraday_data['px_change_rate'])

                        if 'preclose_px' in intraday_data:
                            stock_data['昨收价'] = float(intraday_data['preclose_px'])
                except Exception as e:
                    logger.debug(f"获取intraday数据失败: {e}")

            # 方法3: 使用 get_stock_big_order_intraday 获取大单数据
            if not stock_data.get('主力资金'):
                try:
                    ...
                    if big_order_data and 'big_order_net_total' in big_order_data:
                        stock_data['主力资金'] = float(big_order_data['big_order_net_total'])
                except Exception as e:
                    logger.debug(f"获取大单数据失败: {e}")

            # 方法4: 使用 akshare 个股信息获取换手率和市值（最后手段）
            if not stock_data.get('换手率'):
                try:
                    import akshare as ak
                    stock_info = ak.stock_individual_info_em(symbol=stock_code)
                    if not stock_info.empty:
                        turnover_rate_row = stock_info[stock_info['item'] == '换手率']
                        if not turnover_rate_row.empty:
                            stock_data['换手率'] = float(turnover_rate_row.iloc[0]['value'])

                        market_cap_row = stock_info[stock_info['item'] == '总市值']
                        if not market_cap_row.empty:
                            stock_data['总市值'] = float(market_cap_row.iloc[0]['value'])
                            stock_data['市值_数值'] = float(market_cap_row.iloc[0]['value'])

                        circulating_cap_row = stock_info[stock_info['item'] == '流通市值']
                        if not circulating_cap_row.empty:
                            stock_data['流通市值'] = float(circulating_cap_row.iloc[0]['value'])
                except Exception as e:
                    logger.debug(f"获取akshare个股信息失败: {e}")

        except Exception as e:
            logger.debug(f"补全股票数据失败 {stock_code}: {e}")
    ...
    def get_concept_limit_up_data(
        ...
        由于开盘啦API返回的是"涨停原因板块"而非"概念板块"，
        此方法通过主题关键词重新分类涨停股票来模拟概念板块统计。

        Args:
            date: 日期字符串，格式 'YYYY-MM-DD'
            concept_keywords: 概念关键词映射，如 {'算力': ['算力', '液冷', '数据中心'], ...}
                            如果为None，使用默认的概念关键词

        Returns:
            字典，键为概念名称，值包含：
            - limit_up_count: 该概念涨停股票数
            - limit_up_stocks: 涨停股票列表
            - consecutive_boards: 连板梯队分布
        """
        logger.info(f"按概念关键词获取涨停数据: date={date}")

        # 默认概念关键词定义（精确匹配涨停原因和主题）
        if concept_keywords is None:
            concept_keywords = {
                # 算力：只匹配主题中明确标注的算力相关关键词
                '算力': ['算力', '液冷'],
                # 化工：涨停原因为化工相关
                '化工': ['化工', '磷化工', '钛白粉', '环氧丙烷', '甲醇', '烧碱'],
                # 智能电网：涨停原因为智能电网相关
                '智能电网': ['智能电网', '变压器', '电气设备', '充电桩'],
                # 电力：涨停原因为电力相关（不含绿色电力算力）
                '电力': ['电力', '绿色电力', '水电', '火电'],
                # 人工智能：AI相关
                '人工智能': ['人工智能', 'AI', 'DeepSeek'],
                # 芯片
                '芯片': ['芯片', '半导体', '存储'],
                # 医药
                '医药': ['医药', '医疗', '生物', '创新药'],
        ...
        try:
            ...
            if not sector_ranking or 'sectors' not in sector_ranking:
                logger.warning(f"获取板块排名数据为空: {date}")
                return result
            ...
            for sector in sector_ranking['sectors']:
                ...
                for stock in stocks:
                    # 添加板块信息
                    stock['涨停原因板块'] = sector.get('sector_name', '')
                    all_stocks.append(stock)

            logger.info(f"获取到 {len(all_stocks)} 只涨停股票")

            # 按概念关键词分类
            classified_stocks = set()  # 记录已分类的股票代码
            ...
            for concept, keywords in concept_keywords.items():
                ...
                for stock in all_stocks:
                    ...
                    for kw in keywords:
                        if kw in reason:
                            # 避免重复分类同一只股票到同一概念
                            stock_key = f"{stock_code}_{concept}"
                            if stock_key not in classified_stocks:
                                matched_stocks.append(stock)
                                classified_stocks.add(stock_key)
                            break

                if matched_stocks:
                    result[concept]['limit_up_count'] = len(matched_stocks)
                    result[concept]['limit_up_stocks'] = matched_stocks
                    ...
                    result[concept]['consecutive_boards'] = consecutive_boards

            # 记录统计结果
            for concept, data in result.items():
                if data['limit_up_count'] > 0:
                    logger.info(f"概念 '{concept}': {data['limit_up_count']}只涨停")

            return result

        except Exception as e:
            logger.error(f"按概念关键词获取涨停数据失败: {e}")
            return result
